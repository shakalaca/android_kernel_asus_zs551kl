...
/*[freddy+++][PM] Add suspend fail log to ASUSevtlog when pm_dev_err*/
...
int dpm_suspend_noirq(pm_message_t state)
{
	...
	while (!list_empty(&dpm_late_early_list)) {
		...
		if (async_error) {
			pm_dev_err(dev, state, " async_error", error);
			break;
		}
	}
	mutex_unlock(&dpm_list_mtx);
	...
	if (error) {
		printk("[PM] dpm_suspend_noirq error! state= %s, async_err= %d\n", pm_verb(state.event), error);
