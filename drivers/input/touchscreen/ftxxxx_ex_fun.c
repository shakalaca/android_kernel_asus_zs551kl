...
extern int m_absDifferData[TX_NUM_MAX][RX_NUM_MAX];
//zax 20150112----------------------------------
...
static ssize_t ftxxxx_ftsscaptest_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (IS_ERR(filp)) {
		...
		wake_unlock(&ftxxxx_ts->wake_lock);
		vfree(tmpbuf);
		return sprintf(buf, "open file fail ! \n");
	}
	...
	count += snprintf(tmpbuf + count, BufLen - count, "TestItem Num, 8, RawData Test, 7, %d, %d, 11, 2, ",  TX_NUM, RX_NUM);
	...
	/*count += snprintf(tmpbuf + count, BufLen - count, "PanelDifferTest,20, %d, %d, 62, 1, ", TX_NUM, RX_NUM);
...
/*for (i = 0; i < TX_NUM; i++) {
		...
		for (j = 0; j < RX_NUM; j++) {
			count += snprintf(tmpbuf + count, BufLen - count, "%5d, ", m_absDifferData[i][j]);
			//msleep(2000);
		}
		//msleep(10000);
		count += snprintf(tmpbuf + count, BufLen - count, "\n");
	...
	/*Print RawData End*/
	if (IS_ERR(filp)) {
		printk("[Focal][TOUCH_ERR] %s: 2open /data/TP_Raw_data failed\n", __func__);
		mutex_unlock(&ftxxxx_ts->g_device_mutex);
		wake_unlock(&ftxxxx_ts->wake_lock);
		vfree(tmpbuf);
		return sprintf(buf, "open file fail ! \n");
	...
	pos = 0;
	vfs_write(filp, tmpbuf, count, &pos);
