...
static ssize_t focalTP_virtual_keys_register(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	char *virtual_keys = 	__stringify(EV_KEY) ":" __stringify(KEY_BACK) ":200:2000:20:20" "\n" \

				__stringify(EV_KEY) ":" __stringify(KEY_HOME) ":360:3000:130:35" "\n" \

				__stringify(EV_KEY) ":" __stringify(KEY_MENU)   ":600:2000:20:20" "\n" ;
...
/*static int fts_input_dev_report_key_event(struct ts_event *event, struct ftxxxx_ts_data *data)
{
    int i;
    if( 1 == visual_key_active)
    {
        ...
        if ( (1 == event->touch_point ) &&
             (event->au16_y[0] == data->pdata->key_y_coord))
        {
                for (i = 0; i < 3; i++)
                {
                    if (event->au16_x[0] > (data->pdata->key_x_coords[i] - 25) &&
                        event->au16_x[0] < (data->pdata->key_x_coords[i] + 25))
                    {
                            ...
                            event->au8_touch_event[i] == 2)
                        {
                            ...
                            focal_debug(DEBUG_VERBOSE,"Key%d(%d, %d) DOWN!", i, event->au16_x[0], event->au16_y[0]);
                        }
                        else
                        {
                            input_report_key(data->input_dev, data->pdata->keys[i], 0);
                            focal_debug(DEBUG_VERBOSE,"Key%d(%d, %d) Up!", i, event->au16_x[0], event->au16_y[0]);
                        }
                        break;
                    }
                }
...
int ftxxxx_read_tp_id(void)
{
	...
	ftxxxx_nosync_irq_disable(ftxxxx_ts->client);
//	if (HD_Modle==2){
		err = ftxxxx_read_reg(ftxxxx_ts->client, FTXXXX_REG_VENDOR_ID, &B_VenderID);
...
/*	gpio_set_value(ftxxxx_ts->pdata->lcd0id_gpio, 1);
	...
	printk("[Focal][Touch] Nancy power on \n");
	...
	if (gpio_request(ftxxxx_ts->irq, FTXXXX_INT_PIN_NAME)) {
		...
		printk("[Focal][TOUCH] %s: gpio %d request for interrupt \n", __func__, ftxxxx_ts->irq);
	gpio_direction_input(ftxxxx_ts->irq);

	printk("[Focal][Touch] gpio to input \n");
