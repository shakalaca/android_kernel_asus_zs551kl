...
#define KEY_GESTURE_E 		   		18
#define KEY_GESTURE_C 		   		46
#define KEY_GESTURE_W 		   		17
#define KEY_GESTURE_S 		   		31
#define KEY_GESTURE_V 		   		47
#define KEY_GESTURE_Z 		   		44
#define KEY_GESTURE_DOUBLECLICK     116
#define KEY_GESTURE_SWIPE_UP        103           
...
static ssize_t synaptics_touch_function_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t synaptics_touch_function_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
...
void asus_psensor_disable_touch(bool enable)
{
	if(enable)
	{
		SYNAPTICS_TOUCH_DISABLE = true;
		printk("[Touch] : proximity trigger disable touch !\n");
	} 
	else
	{
		SYNAPTICS_TOUCH_DISABLE = false;
		printk("[Touch] : proximity trigger enable touch !\n");
	}
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if(device_status.status_code == 0)
	{
		return snprintf(buf, PAGE_SIZE, "%d\n",1);
	}
	else
	{
		printk("[TOUCH]: %s : Touch IC Status is %d\n",__func__,device_status.status_code);
		return snprintf(buf, PAGE_SIZE, "%d\n",0);	
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	dclick_mode = input;
	printk("[SYNAP][TOUCH] dclick_mode: %d\n",dclick_mode);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	gesture_mode = value;
	printk("[SYNAP][TOUCH] gesture_mode: %d\n",gesture_mode);
...
static void synaptics_check_gesture(struct synaptics_rmi4_data *rmi4_data,int gesture_type,int gesture_alphabet)
{
	printk("[Touch] : gesture_type is %d, gesture_alphabet is %d\n",gesture_type,gesture_alphabet);
	switch(gesture_type)
	{
		case GESTURE_DOUBLECLICK:
			if(dclick_mode == 1)
			{
				printk("[Touch] double click wake up\n");
				input_report_key(rmi4_data->input_dev, KEY_GESTURE_DOUBLECLICK, 1);
				input_sync(rmi4_data->input_dev);
				input_report_key(rmi4_data->input_dev, KEY_GESTURE_DOUBLECLICK, 0);
				input_sync(rmi4_data->input_dev);
				//rmi4_data->suspend = false;
			}
			break;
		...
		case GESTURE_VEE_REG:
			...
			{
				printk("[Touch] gesture v wake up\n");
		...
		case GESTURE_OTHERS:
			if (gesture_alphabet == GESTURE_E) 
			{
				...
				{
					printk("[Touch] gesture e wake up\n");
			...
			else if (gesture_alphabet == GESTURE_C) 
			{
				...
				{
					printk("[Touch] gesture c wake up\n");
			...
			else if (gesture_alphabet == GESTURE_W) 
			{
				...
				{
					printk("[Touch] gesture w wake up\n");
			...
			else if (gesture_alphabet == GESTURE_S) 
			{
				...
				{
					printk("[Touch] gesture s wake up\n");
			...
			else if (gesture_alphabet == GESTURE_Z) 
			{
				...
				{
					printk("[Touch] gesture z wake up\n");
			...
			break;
			default:
				printk("[Touch] gesture can't parse\n");
				break;		
	}
}
...
//extern int report_hall_status(void);
...
flash_prog_mode:
	...
	asus_config_id[0] = config_id[0];
	asus_config_id[1] = config_id[1];
	asus_config_id[2] = config_id[2];
	asus_config_id[3] = config_id[3];
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture) {
			set_bit(KEY_GESTURE_DOUBLECLICK, rmi4_data->input_dev->keybit);
			input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_GESTURE_DOUBLECLICK);
			...
			set_bit(KEY_GESTURE_C, rmi4_data->input_dev->keybit);
			input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_GESTURE_C);
			set_bit(KEY_GESTURE_E, rmi4_data->input_dev->keybit);
			input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_GESTURE_E);
			set_bit(KEY_GESTURE_S, rmi4_data->input_dev->keybit);
			...
			set_bit(KEY_GESTURE_Z, rmi4_data->input_dev->keybit);
...
exit:
	...
	if (!list_empty(&exp_data.list)) {
		list_for_each_entry(exp_fhandler, &exp_data.list, link)
			if (exp_fhandler->exp_fn->suspend != NULL)
				exp_fhandler->exp_fn->suspend(asus_rmi4_data);
	}
	mutex_unlock(&exp_data.mutex);

	asus_rmi4_data->suspend = true;
	printk("[Touch] : asus_rmi4_suspend end to suspend\n");
	return 0;
}
...
exit:
	printk("[Touch] : asus_rmi4_resume begin exit\n");
	...
	if (!list_empty(&exp_data.list)) {
		list_for_each_entry(exp_fhandler, &exp_data.list, link)
			if (exp_fhandler->exp_fn->resume != NULL)
				exp_fhandler->exp_fn->resume(asus_rmi4_data);
	}
	mutex_unlock(&exp_data.mutex);

	asus_rmi4_data->suspend = false;

	printk("[Touch] : asus_rmi4_resume end exit\n");
	return 0;
}
EXPORT_SYMBOL(asus_rmi4_resume);
