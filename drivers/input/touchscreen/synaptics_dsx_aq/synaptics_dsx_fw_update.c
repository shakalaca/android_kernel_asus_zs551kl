...
unsigned int  asus_firmware_id = 0;
unsigned char asus_config_id[4] = {0};
EXPORT_SYMBOL(asus_config_id);
...
static struct device_attribute attrs[] = {
	__ATTR(tpfw_version, S_IRUGO,
			fwu_sysfs_tpfw_version_show,
...
static enum flash_area fwu_go_nogo(void)
{
	...
	asus_firmware_id = device_fw_id;

	if (image_fw_id != device_fw_id) {
		asus_firmware_id = image_fw_id;
		flash_area = UI_FIRMWARE;
		goto exit;
	} else{
		dev_info(rmi4_data->pdev->dev.parent,
				"%s: Image firmware ID equal device firmware ID\n",
				__func__);
		flash_area = NONE;
	}
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->image == NULL) {
		...
		AQU_BOE_FW_IMAGE_NAME, sizeof(AQU_BOE_FW_IMAGE_NAME),
		sizeof(AQU_BOE_FW_IMAGE_NAME));					
printk("[Touch] : AQU BOE FW_IMAGE_NAME is %s\n",fwu->image_name);
		/*switch(asus_project_id)
		{
			case 4 :
					switch(asus_lcd_id)
					{
						...
						case 3 :
							retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
							AQU_TM_FW_IMAGE_NAME, sizeof(AQU_TM_FW_IMAGE_NAME),
							sizeof(AQU_TM_FW_IMAGE_NAME));
							printk("[Touch] : AQU TM FW_IMAGE_NAME is %s\n",fwu->image_name);
							break;
						default :
							retval = -1;
							printk("[Touch] No this Panel !\n");
							break;
					}
				break;
			case 5 :
					switch(asus_lcd_id)
					{
						case 1 :
							retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
							PISCES_BOE_FW_IMAGE_NAME, sizeof(PISCES_BOE_FW_IMAGE_NAME),
							sizeof(PISCES_BOE_FW_IMAGE_NAME));
							printk("[Touch] : PISCES BOE FW_IMAGE_NAME is %s\n",fwu->image_name);
							break;
						case 3 :
							retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
							PISCES_TM_FW_IMAGE_NAME, sizeof(PISCES_TM_FW_IMAGE_NAME),
							sizeof(PISCES_TM_FW_IMAGE_NAME));
							printk("[Touch] : PISCES TM FW_IMAGE_NAME is %s\n",fwu->image_name);
							break;
						default :
							retval = -1;
							printk("[Touch] No this Panel !\n");
...
static int fwu_recovery_write_chunk(void)
{
	...
	pr_notice("%s: chunk_total = %d\n", __func__, chunk_total);
...
static int fwu_start_recovery(void)
{
	...
	if (fwu->image == NULL) {
retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
							BOE_FW_IHEX_NAME, sizeof(BOE_FW_IHEX_NAME),
							sizeof(BOE_FW_IHEX_NAME));
							printk("[Touch] : AQU BOE FW_IMAGE_NAME is %s\n",fwu->image_name);
		/*switch(asus_lcd_id)
		{
			case 1 :
			...
			case 3 :
				retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
							TM_FW_IHEX_NAME, sizeof(TM_FW_IHEX_NAME),
							sizeof(TM_FW_IHEX_NAME));
							printk("[Touch] : AQU TM FW_IMAGE_NAME is %s\n",fwu->image_name);
				break;
			default :
				retval = -1;
...
static ssize_t fwu_sysfs_tpfw_version_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf,PAGE_SIZE,"%u_%02x%02x%02x%02x\n",asus_firmware_id,asus_config_id[0],asus_config_id[1],
	asus_config_id[2],asus_config_id[3]);
}
...
static void synaptics_rmi4_fwu_reset(struct synaptics_rmi4_data *rmi4_data)
{
	...
	for (retries = 0; retries < 5; retries++) {
		...
		if (fwu->ubl_status == 5) {
			fwu_recovery_reset();
			printk("synaptics_rmi4_fwu_reset : recovery reset %d\n", retries + 1);
			continue;
		}
	}
	if (retval < 0)
