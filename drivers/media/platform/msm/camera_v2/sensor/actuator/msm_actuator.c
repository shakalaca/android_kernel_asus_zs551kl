...
/*For ASUS_BSP ZZ VCM+++*/
static int rear_vcm_read(struct seq_file *buf, void *v)
{
	int ret;
	uint16_t reg_val = 0xEEEE; //ASUS_BSP ZZ++
	
	ret =  rear_vcm_ctrl_t->i2c_client.i2c_func_tbl->i2c_read(
			&rear_vcm_ctrl_t->i2c_client,
			rear_vcm_value,
			&reg_val,
			MSM_CAMERA_I2C_BYTE_DATA);

	seq_printf(buf,"Camera 0 reg 0x%04x val 0x%x\n",rear_vcm_value,reg_val);

	return ret;
}

static int rear_vcm_open(struct inode *inode, struct  file *file)
{
	return single_open(file, rear_vcm_read, NULL);
}

static ssize_t rear_vcm_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
	ssize_t ret_len;
	int n;
	char messages[32]="";
	...
	else if( n == 2)
	{
		...
		rear_vcm_value = val[0];
		g_camera_reg_val = val[1];
		g_camera_sensor_operation = 1;
	}

	if(g_camera_sensor_operation == 1)
	{
		rc =  rear_vcm_ctrl_t->i2c_client.i2c_func_tbl->i2c_write(
			&rear_vcm_ctrl_t->i2c_client,
			rear_vcm_value,
			g_camera_reg_val,
			MSM_CAMERA_I2C_BYTE_DATA
		...
		if(rc < 0)
		{
			pr_err("write 0x%x to camera id 0 addr 0x%04x FAIL\n",g_camera_reg_val,rear_vcm_value);
		}
		else
		{
			pr_err("write 0x%x to camera id 0 addr 0x%04x OK\n",g_camera_reg_val,rear_vcm_value);
		}
	}
RETURN:
	return ret_len;
...
static const struct file_operations rear_vcm_proc_fops = {
	...
	.open = rear_vcm_open,
	.read = seq_read,
	.write = rear_vcm_write,
	.llseek = seq_lseek,
	.release = single_release,
};

static int rear_vcm_2_read(struct seq_file *buf, void *v)
{
	int ret;
	uint16_t reg_val = 0xEEEE; //ASUS_BSP ZZ++
...
static int rear_vcm_2_open(struct inode *inode, struct  file *file)
{
	return single_open(file, rear_vcm_2_read, NULL);
}

static ssize_t rear_vcm_2_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
	ssize_t ret_len;
	int n;
	char messages[32]="";
	...
	if(g_camera_sensor_operation == 1)
	{
		rc =  rear_vcm2_ctrl_t->i2c_client.i2c_func_tbl->i2c_write(
			&rear_vcm2_ctrl_t->i2c_client,
			rear_vcm_value_2,
			g_camera_reg_val,
			MSM_CAMERA_I2C_BYTE_DATA
		...
		if(rc < 0)
		{
			pr_err("write 0x%x to camera id 2 addr 0x%04x FAIL\n",g_camera_reg_val,rear_vcm_value_2);
		}
		else
		{
			pr_err("write 0x%x to camera id 2 addr 0x%04x OK\n",g_camera_reg_val,rear_vcm_value_2);
		}
	}
RETURN:
	return ret_len;
...
static const struct file_operations rear_vcm_2_proc_fops = {
	.owner = THIS_MODULE,
	.open = rear_vcm_2_open,
	.read = seq_read,
	.write = rear_vcm_2_write,
	.llseek = seq_lseek,
	.release = single_release,
};
...
/*For ASUS_BSP ZZ VCM+++*/
static int front_vcm_read(struct seq_file *buf, void *v)
{
	int ret;
	uint16_t reg_val = 0xEEEE; //ASUS_BSP ZZ++
	
	ret =  front_vcm_ctrl_t->i2c_client.i2c_func_tbl->i2c_read(
			&front_vcm_ctrl_t->i2c_client,
			front_vcm_value,
			&reg_val,
			MSM_CAMERA_I2C_BYTE_DATA);

	seq_printf(buf,"Camera 1 reg 0x%04x val 0x%x\n",front_vcm_value,reg_val);

	return ret;
}

static int front_vcm_open(struct inode *inode, struct  file *file)
{
	return single_open(file, front_vcm_read, NULL);
}

static ssize_t front_vcm_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
	ssize_t ret_len;
	int n;
	char messages[32]="";
	...
	if(g_camera_sensor_operation == 1)
	{
		rc =  front_vcm_ctrl_t->i2c_client.i2c_func_tbl->i2c_write(
			&front_vcm_ctrl_t->i2c_client,
			front_vcm_value,
			g_camera_reg_val,
			MSM_CAMERA_I2C_BYTE_DATA
		...
		if(rc < 0)
		{
			pr_err("write 0x%x to camera id 1 addr 0x%04x FAIL\n",g_camera_reg_val,front_vcm_value);
		}
		else
		{
			pr_err("write 0x%x to camera id 1 addr 0x%04x OK\n",g_camera_reg_val,front_vcm_value);
		}
	}
RETURN:
	return ret_len;
...
//	for (i = (a_ctrl->cam_name == ACTUATOR_MAIN_CAM_2)? size-1 : 0; i < size; i++) { //ASUS_BSP ZZ++
	for (i = 0; i < size; i++) { //ASUS_BSP Randy++
		...
		case MSM_ACT_WRITE:
			/*ASUS_BSP ZZ++*/
			/*
			pr_err("%s:%d Write addr = 0X%X, data = 0X%X, dt = %d cam_name=%d size=%d",
		...
		case MSM_ACT_CHECK:
			rc = a_ctrl->i2c_client.i2c_func_tbl->i2c_read(
				...
				&reg_val,
				settings[i].data_type);
			if(reg_val != settings[i].reg_data){
				i = REAR_VCM_CHECK_KVALUE_COUNT-1;
				continue;
			}else {
				cur_rear_vcm_check_count++;
			}
			if(cur_rear_vcm_check_count == REAR_VCM_CHECK_KVALUE_COUNT){
				i = size-2;
				continue;
			}
			/*
			pr_err("%s:%d Check addr = 0X%X, data = 0X%X, dt = %d cam_name=%d size=%d reg_val=%d check_count=%d",
				__func__, __LINE__, settings[i].reg_addr,
				settings[i].reg_data, settings[i].data_type,
				a_ctrl->cam_name,
				size,
	...
	struct msm_actuator_set_position_t *set_pos)
{
	...
	if (a_ctrl->actuator_state != ACT_OPS_ACTIVE) {
		...
		/*ASUS_BSP ZZ++*/
		if(a_ctrl->cam_name == ACTUATOR_MAIN_CAM_0 &&  rear_vcm_active_mode ==  false){
			rc =  a_ctrl->i2c_client.i2c_func_tbl->i2c_write(
				&a_ctrl->i2c_client,
				0x02,
				0x00,
			...
			rear_vcm_active_mode = true;
			pr_err("Set vcm to active mode cameraId=%d rc=%d\n", a_ctrl->cam_name, rc);
		}else if(a_ctrl->cam_name == ACTUATOR_MAIN_CAM_2 &&  rear_vcm2_active_mode ==  false ){
			rc =  a_ctrl->i2c_client.i2c_func_tbl->i2c_write(
				&a_ctrl->i2c_client,
				0x02,
				0x00,
	...
	for (index = 0; index < set_pos->number_of_steps; index++) {
		...
		/*For ASUS_BSP ZZ VCM+++*/
		//printk("\n[actuator] msm_actuator_set_position  %s next_lens_position:%d, a_ctrl->cam_name = %d\n",__func__,next_lens_position, a_ctrl->cam_name);
	...
	struct msm_actuator_set_position_t *set_pos)
{
	...
	for (index = 0; index < set_pos->number_of_steps; index++) {
		next_lens_position = set_pos->pos[index];
		/*For ASUS_BSP ZZ VCM+++*/
		//printk("\n[actuator] msm_actuator_bivcm_set_position %s next_lens_position:%d, a_ctrl->cam_name = %d\n",__func__,next_lens_position, a_ctrl->cam_name);
		if(a_ctrl->cam_name == ACTUATOR_MAIN_CAM_0 ){
			rear_vcm_value = next_lens_position;
		}else{
			rear_vcm_value_2 = next_lens_position;
...
static int msm_actuator_init(struct msm_actuator_ctrl_t *a_ctrl)
{
	...
	if(a_ctrl->cam_name == ACTUATOR_MAIN_CAM_0 ){
		...
		cur_rear_vcm_check_count = 0;
		rear_vcm_active_mode = false;
	}else if(a_ctrl->cam_name == ACTUATOR_MAIN_CAM_2 ){
		rear_vcm2_ctrl_t = a_ctrl;
		rear_vcm2_active_mode = false;
	}else if(a_ctrl->cam_name == ACTUATOR_MAIN_CAM_1 ){
		front_vcm_ctrl_t = a_ctrl;
		 front_vcm_active_mode = false;
	}
	//ASUS_BSP ZZ++
	CDBG("Exit\n");
	return rc;
...
static int msm_actuator_close(struct v4l2_subdev *sd,
	...
	a_ctrl->actuator_state = ACT_DISABLE_STATE;
	mutex_unlock(a_ctrl->actuator_mutex);
	//ASUS_BSP ZZ clear value after close+++
	rear_vcm_value=0;
	rear_vcm_value_2=0;
	front_vcm_value=0;
	//ASUS_BSP ZZ clear value after close---
...
static int32_t msm_actuator_platform_probe(struct platform_device *pdev)
{
	...
	if( msm_actuator_t->cam_name == ACTUATOR_MAIN_CAM_1){
		proc_create_data("driver/front_vcm", 0666, NULL, &front_vcm_proc_fops, NULL);
	}else if(msm_actuator_t->cam_name == ACTUATOR_MAIN_CAM_0){
