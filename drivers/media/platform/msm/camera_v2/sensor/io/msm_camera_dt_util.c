...
DEFINE_MSM_MUTEX(msm_gpio_mutex); //ASUS_BSP Deka +++
int gpio20_ref_cnt = 0;
unsigned gpio20_num = -1;
void gpio_switch_value_with_ref(unsigned gpio, int value)
{
    mutex_lock(&msm_gpio_mutex); //ASUS_BSP Deka+++
    if(gpio20_num == -1){
        gpio20_num = gpio;
    }
    BUG_ON(gpio20_num != gpio);
    if( value > 0){
        gpio20_ref_cnt ++;
        if(gpio20_ref_cnt == 1){
        ...
        if(gpio20_ref_cnt < 0){
            pr_err("%s [warning] gpio20_ref_cnt %d reset to 0", __func__, gpio20_ref_cnt);
            gpio20_ref_cnt = 0;
        }
    }
    pr_err("%s gpio20_ref_cnt %d [%d] %d", __func__, gpio20_ref_cnt, gpio, value);
		...
		case SENSOR_VREG:
			...
			if (ctrl->gpio_conf->cam_gpio_req_tbl && ctrl->gpio_conf->cam_gpio_req_tbl_size) { //ASUS_BSP PJ_Ma +++ "add to check gpio pin if been defined in dtsi"
				...
				if (rc < 0) {
                    ...
                    #endif //ASUS_BSP Deka --- "add ref cnt for gpio 20" //Temp disable
			break;
		case SENSOR_I2C_MUX:
		...
		case SENSOR_GPIO:
			...
			if (!ctrl->gpio_conf->gpio_num_info->valid
				...
				continue;
        				...
        				[pd->seq_val],
        				(int) pd->config_val);
                            pr_err("Deka check gpio msm_camera_power_up %d gpio_offset %d set %d", ctrl->gpio_conf->gpio_num_info->gpio_num[pd->seq_val], pd->seq_val, (int) pd->config_val);
                        }else{
                            gpio_set_value_cansleep(
        				ctrl->gpio_conf->gpio_num_info->gpio_num
        				[pd->seq_val],
