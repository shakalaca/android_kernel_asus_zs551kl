...
/* ASUS_BSP (ShowCai) +++ Get charger temp retry */
#define TADC_RETRY_TIMES 3
/* ASUS_BSP (ShowCai) --- Get charger temp retry */
				 ...
				 union power_supply_propval *val)
{
	...
	for (i = 0; i < die_avg_count; i++) {
		...
		if (rc < 0) {
			/* ASUS_BSP (ShowCai) +++ Get charger temp retry */
			if(rc == -ETIMEDOUT){
				for (j = 0; j < TADC_RETRY_TIMES; j++ ){
					rc = smblib_get_prop_charger_temp(chg, &pval);
					if(rc < 0){
						pr_err("Couldnt read chg temp at %dth iteration rc = %d: Retry %d\n",
							i + 1, rc, j + 1);
					}else{
						pr_err("Read chg temp success at %dth iteration rc = %d(retry:%d)\n",
							i + 1, rc, j + 1);
						break;
					}
				}
				if(j == TADC_RETRY_TIMES){
...
/* ASUS_BSP (ShowCai) +++ add charger mtemp node */
static int smb138x_get_prop_connector_temp(struct smb138x *chip,
				 union power_supply_propval *val)
{
	struct smb_charger *chg = &chip->chg;
	int rc, connector_mdegc;
...
static int smb138x_parse_dt(struct smb138x *chip)
{
	...
	chip->dt.batfet_en_gpio = of_get_named_gpio(node,
				"qcom,smb-batfet-en", 0);
	if (!gpio_is_valid(chip->dt.batfet_en_gpio))
		chip->dt.batfet_en_gpio = 0;
...
static enum power_supply_property smb138x_parallel_props[] = {
	...
	POWER_SUPPLY_PROP_CHARGER_TEMP,
	/* ASUS_BSP (ShowCai) +++ add charger mtemp node */
	POWER_SUPPLY_PROP_CHARGER_MTEMP,
	/* ASUS_BSP (ShowCai) --- add charger mtemp node */
	POWER_SUPPLY_PROP_CHARGER_TEMP_MAX,
				     ...
				     union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGER_TEMP:
		...
		break;
	/* ASUS_BSP (ShowCai) +++ add charger mtemp node */
	case POWER_SUPPLY_PROP_CHARGER_MTEMP:
		rc = smb138x_get_prop_connector_temp(chip, val);
		break;
	/* ASUS_BSP (ShowCai) --- add charger mtemp node */
	case POWER_SUPPLY_PROP_CHARGER_TEMP_MAX:
		rc = smblib_get_prop_charger_temp_max(chg, val);
