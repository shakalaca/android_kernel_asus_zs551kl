				  ...
				  u32 param_size)
{
	...
	if ((desc_id != QUERY_DESC_IDN_STRING) &&
		...
		//ASUS_BSP Deeo : QUERY_DESC_LENGTH_OFFSET of UFS 2.0 is different with UFS 2.1 +++
		//ret = -EINVAL;
		//goto out;
		//ASUS_BSP Deeo : QUERY_DESC_LENGTH_OFFSET of UFS 2.0 is different with UFS 2.1 ---
	}
...
/*
static ssize_t ufshcd_ufs_rev_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct ufs_hba *hba = dev_get_drvdata(dev);
	int curr_len;

	printk("[UFS] Vendor : %c%c%c%c%c%c%c%c\n", hba->vendor[0], hba->vendor[1], hba->vendor[2], hba->vendor[3], hba->vendor[4], hba->vendor[5], hba->vendor[6], hba->vendor[7]);
	printk("[UFS] Model : %c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c\n", hba->model[0], hba->model[1], hba->model[2], hba->model[3], hba->model[4], hba->model[5], hba->model[6], hba->model[7],
																									  hba->model[8], hba->model[9], hba->model[10], hba->model[11], hba->model[12], hba->model[13], hba->model[14], hba->model[15]);
	curr_len = snprintf(buf, PAGE_SIZE,"%c%c%c%c\n", hba->rev[0], hba->rev[1], hba->rev[2], hba->rev[3]);
	return curr_len;
}

static ssize_t ufshcd_ufs_rev_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	return count;
}

static void ufshcd_add_ufs_rev_sysfs_nodes(struct ufs_hba *hba)
{
	hba->ufs_rev_attr.show = ufshcd_ufs_rev_show;
	hba->ufs_rev_attr.store = ufshcd_ufs_rev_store;
	sysfs_attr_init(&hba->ufs_rev_attr.attr);
	hba->ufs_rev_attr.attr.name = "ufs_rev";
	hba->ufs_rev_attr.attr.mode = S_IRUGO | S_IWUSR;
	if (device_create_file(hba->dev, &hba->ufs_rev_attr))
		dev_err(hba->dev, "Failed to create sysfs for ufs_rev\n");
}
*/
...
static inline void ufshcd_add_sysfs_nodes(struct ufs_hba *hba)
{
	...
	//ufshcd_add_ufs_rev_sysfs_nodes(hba);
