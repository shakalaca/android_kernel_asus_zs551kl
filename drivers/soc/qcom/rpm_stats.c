		...
		struct msm_rpm_stats_data_v2 *data, int buflength)
{
	...
	return snprintf(buf, buflength,
		...
		"Client votes: %llx, Subsystem votes: 0x%x\n\n",
			...
			struct msm_rpmstats_private_data *prvdata)
{
	...
	for (i = 0, length = 0; i < prvdata->num_records; i++) {
		...
		data.accumulated = msm_rpmstats_read_quad_register_v2(reg,
				i, offsetof(struct msm_rpm_stats_data_v2,
					accumulated));
		data.client_votes = msm_rpmstats_read_quad_register_v2(reg,
				i, offsetof(struct msm_rpm_stats_data_v2,
					client_votes));
...
static int rpm_stats_suspend(struct device *dev)
{
	...
	printk("[RPM] Suspend: Voting status: Client: 0x%llx, Subsystem: 0x%0x\n",
			data.client_votes, data.subsystem_votes);
...
#define SUBSYSTEM_VOTES_ADSP (1 << 12)
u32  this_count = 0;
...
static int rpm_stats_resume(struct device *dev)
{
	...
	u64 time_since_last_mode;
	u64 actual_last_sleep;
	static bool previous_sleepless = false;
	bool current_sleepless = false;
	...
	for (i = 0, length = 0; i < 2; i++) {
		...
		if(0 == i) {
			...
			printk("[RPM] Resume: Client votes: 0x%llx, Subsystem votes: 0x%x\n",
				data.client_votes, data.subsystem_votes);

			current_sleepless = !!(data.subsystem_votes & SUBSYSTEM_VOTES_ADSP);
			if(current_sleepless & previous_sleepless) {
				adsp_sleep_count++;
				printk("[RPM]%s: Increase ADSP sleep counter: %u\n", __func__, adsp_sleep_count);
			} else {
				adsp_sleep_count = 0;
				if(!current_sleepless & previous_sleepless) {
					printk("[RPM]%s: ADSP sleep status changed! Reset ADSP sleep counter: %u\n", __func__, adsp_sleep_count);
				}
			}

			previous_sleepless = current_sleepless;
