...
static int dwc3_gadget_pullup(struct usb_gadget *g, int is_on)
{
	 ...
	 * during enumeration.
	 */
	dev_info(dwc->dev, "[USB][GADGET] Notify pull-up from %s\n", __func__);
	dwc->b_suspend = false;
...
static void dwc3_gadget_disconnect_interrupt(struct dwc3 *dwc)
{
	...
	dev_info(dwc->dev, "[USB][GADGET] Notify disconnect from %s\n", __func__);
	dwc->b_suspend = false;
	dwc3_notify_event(dwc, DWC3_CONTROLLER_NOTIFY_OTG_EVENT, 0);
...
static void dwc3_gadget_reset_interrupt(struct dwc3 *dwc)
{
	...
	dev_info(dwc->dev, "[USB][GADGET] Notify reset from %s\n", __func__);
	dwc->b_suspend = false;
	dwc3_notify_event(dwc, DWC3_CONTROLLER_NOTIFY_OTG_EVENT, 0);
...
static void dwc3_gadget_wakeup_interrupt(struct dwc3 *dwc, bool remote_wakeup)
{
	...
	dev_info(dwc->dev, "[USB][GADGET] Wakeup interrupt from %s\n", __func__);
			...
			unsigned int evtinfo)
{
	...
	dev_info(dwc->dev, "[USB][GADGET] Suspend interrupt : %s Entry to %d\n", __func__, next);
