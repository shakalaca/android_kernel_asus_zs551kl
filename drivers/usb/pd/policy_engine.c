...
#define ASUS_PD_MAX_INPUT_VOL_CFG 9000	// 9V
#define ASUS_PD_MID_INPUT_VOL_CFG 6000	// 6V
#define ASUS_PD_MIN_INPUT_VOL_CFG 5000	// 5V
#define ASUS_PD_ICL_CUR_CFG_3A 3000		// 3A
#define ASUS_PD_ICL_CUR_CFG_2A 2000		// 2A
#endif
...
static int pd_eval_src_caps_asus(struct usbpd *pd)
{
	union power_supply_propval val;
	int i;
	int cur_mv, cur_ma, cur_mw, index = 0;
	int pre_mv = 0, pre_ma = 0, pre_mw = 0;
	...
	for (i = 0; i < pd->src_cap_cnt; i++) {
		...
		cur_mv = PD_SRC_PDO_FIXED_VOLTAGE(pd->received_pdos[i]) * 50;
		cur_ma = PD_SRC_PDO_FIXED_MAX_CURR(pd->received_pdos[i]) * 10;

		if ( cur_mv >= ASUS_PD_MIN_INPUT_VOL_CFG && cur_mv <= ASUS_PD_MID_INPUT_VOL_CFG)
			cur_ma = (cur_ma > ASUS_PD_ICL_CUR_CFG_3A) ? ASUS_PD_ICL_CUR_CFG_3A : cur_ma;
		else if (cur_mv <= ASUS_PD_MAX_INPUT_VOL_CFG)
			cur_ma = (cur_ma > ASUS_PD_ICL_CUR_CFG_2A) ? ASUS_PD_ICL_CUR_CFG_2A : cur_ma;
		else if (cur_mv > ASUS_PD_MAX_INPUT_VOL_CFG)
			cur_ma = 0;
		else {
			usbpd_err(&pd->dev, "cur_mv %d invalid\n", cur_mv);
			continue;
		}

		cur_mw = cur_mv * cur_ma;
		if (pre_mw < cur_mw || (pre_mw == cur_mw && cur_mv < pre_mv)) {
			pre_mw = cur_mw;
			pre_mv = cur_mv;
			pre_ma = cur_ma;
			index = i;
	...
	usbpd_info(&pd->dev, "select index=%d mv=%d ma=%d\n", index, pre_mv, pre_ma);
	...
	pd_select_pdo(pd, index + 1, pre_mv * 1000, pre_ma * 1000);
...
static void usbpd_set_state(struct usbpd *pd, enum usbpd_state next_state)
{
	...
	case PE_SNK_STARTUP:
		...
		if ((!val.intval && !pd->in_pr_swap) || disable_usb_pd) {
...
static void usbpd_sm(struct work_struct *w)
{
	...
	if (pd->current_pr == PR_NONE) {
		...
		pd->peer_pr_swap = false;
		pd->peer_dr_swap = false;
		pd->caps_count = 0;
		pd->hard_reset_count = 0;
...
static int psy_changed(struct notifier_block *nb, unsigned long evt, void *ptr)
{
	...
	case POWER_SUPPLY_TYPEC_SINK:
		...
		if (pd->current_state == PE_PRS_SRC_SNK_WAIT_SOURCE_ON
				|| pd->current_state == PE_PRS_SRC_SNK_TRANSITION_TO_OFF
				|| pd->current_state == PE_PRS_SNK_SRC_SOURCE_ON) {
			usbpd_info(&pd->dev, "ingnor pd state = %d\n", pd->current_state);
			return 0;
		}
		...
		enum dual_role_property prop)
{
	...
	case DUAL_ROLE_PROP_DR:
		if (pd->current_pr == PR_SINK) {
			ret = (pd->peer_dr_swap == true);
			break;
		}

	case DUAL_ROLE_PROP_PR:
		if (pd->current_pr == PR_SINK)
			ret = (pd->peer_pr_swap == true);
		else if (pd->current_pr == PR_SRC)
			ret = (pd->current_state == PE_SRC_READY);
...
#ifdef CONFIG_ASUS_PD_CHARGER
	usbpd_info(&pd->dev, "don't select pdo by userspace\n");
	return size;
#endif
	mutex_lock(&pd->swap_lock);
