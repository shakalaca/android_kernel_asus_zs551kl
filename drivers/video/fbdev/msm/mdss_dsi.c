...
#include <linux/kernel.h>   /*ASUS BSP Display+++*/
...
bool first_panel_power_on = true;
struct mdss_panel_data *g_mdss_pdata;
extern void set_alpm(bool enable); //add for alpm
//ASUS BSP Display ---
...
extern void ftxxxx_ts_resume(void); // nancy+++
...
static int mdss_dsi_panel_power_lp(struct mdss_panel_data *pdata, int enable)
{
	...
	//ASUS BSP Bernard, add for alpm +++
	pr_err("[Display] %s: alpm (%d)\n", __func__, enable);
	if (enable) {
		set_alpm(true);
