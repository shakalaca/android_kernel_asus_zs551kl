...
extern bool first_panel_power_on;
							...
							u32 bl_level)
{
	...
	if (bl_level != 0) {
		if ( (bl_level <= 6 && g_previous_bl > 6) || (bl_level <= 6 && g_previous_bl== 0) ) {
			pr_err("[Display] %s:low bklt, level = %d\n", __func__, bl_level);
			mdss_dsi_panel_cmds_send(ctrl_pdata, &ctrl_pdata->low_bklt_cmds, CMD_REQ_COMMIT);
		} else if ( (bl_level > 6) && (g_previous_bl <= 6) && (g_previous_bl != 0) ) {
			pr_err("[Display] %s:high bklt, level = %d\n", __func__, bl_level);
			mdss_dsi_panel_cmds_send(ctrl_pdata, &ctrl_pdata->high_bklt_cmds, CMD_REQ_COMMIT);
		}
   	 }
	// ASUS BSP Display, add for panel bklt issue ---
	...
	/* ASUS BSP Display, add for bklt workaround +++ */
	spin_lock(&bklt_lock);
	if (g_previous_bl == 0) {
		g_timer = true;
		wq = true;
...
void set_tcon_cmd(char *cmd, short len)
{
    ...
    for(i=0; i<len && PANEL_DEBUG_CMD; i++)
...
static ssize_t unique_id_write(struct file *filp, const char *buff, size_t len, loff_t *off)
{
    char messages[256];
    ...
    if (strncmp(messages, "id", 1) == 0) {
        pr_info("[Display] dump lcd unique id\n");
        /*dump_uniqueId();*/
    }
...
static ssize_t unique_id_read(struct file *file, char __user *buf,
                    size_t count, loff_t *ppos)
{
    int len = 0;
    ssize_t ret = 0;
    ...
    if (*lcd_unique_id == '\0')
        pr_info("[Display] lcd unique id is empty\n");
...
static struct file_operations lcd_uniqueID_ops = {
    .write = unique_id_write,
    .read = unique_id_read,
};

static void set_hbm_mode(int mode)
{
    struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
    ctrl_pdata = container_of(g_mdss_pdata, struct mdss_dsi_ctrl_pdata, panel_data);
...
static ssize_t hbm_mode_write(struct file *filp, const char *buff, size_t len, loff_t *off)
{
    ...
    if (g_alpm_mode == 0) {
        ...
        pr_err("[Display] always on is enable +++\n");
...
void set_alpm_mode(int mode)
{
    ...
    if (mode==0) {
        pr_err("[Display] alpm off\n");
        g_hbm_mode = 0; // set hbm mode to 0 for enabling hbm function
        mdss_dsi_panel_cmds_send(ctrl_pdata, &ctrl_pdata->alpm_off_cmds, CMD_REQ_COMMIT);
    } else if (mode==1) {
        pr_err("[Display] alpm on to 40 nits from off\n");
        g_hbm_mode = 2; // set hbm mode to 2 for blocking hbm function
        mdss_dsi_panel_cmds_send(ctrl_pdata, &ctrl_pdata->alpm_40nits_cmds, CMD_REQ_COMMIT);
    } else if (mode==2) {
        pr_err("[Display] alpm on to 5 nits from off\n");
        mdss_dsi_panel_cmds_send(ctrl_pdata, &ctrl_pdata->alpm_5nits_cmds, CMD_REQ_COMMIT);
    } else if (mode==3) {
        pr_err("[Display] alpm on to 40 nits from 5 nits\n");
        mdss_dsi_panel_cmds_send(ctrl_pdata, &ctrl_pdata->alpm_40nits_cmds, CMD_REQ_COMMIT);
    } else if (mode==4) {
        pr_err("[Display] alpm on to 5 nits from 40 nits\n");
        mdss_dsi_panel_cmds_send(ctrl_pdata, &ctrl_pdata->alpm_5nits_cmds, CMD_REQ_COMMIT);
    }
}
EXPORT_SYMBOL(set_alpm_mode);
...
static ssize_t alpm_mode_write(struct file *filp, const char *buff, size_t len, loff_t *off)
{
    ...
    if (strncmp(messages, "0", 1) == 0) {
        ...
        pr_err("[Display] does not match any alpm mode.\n");
    }
...
void set_alpm(bool enable)
{
    struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
    char pwr_cmd[2] = {0x0A, 0x00};
    char *rx_buf;
    int i = 0;

    ctrl_pdata = container_of(g_mdss_pdata, struct mdss_dsi_ctrl_pdata, panel_data);
    rx_buf = kmalloc(sizeof(ctrl_pdata->rx_buf.len), GFP_KERNEL);

    if (enable) {
        while (rx_buf[0] != 0xDC && i<10) {
            msleep(VSYNC_DELAY);
            set_alpm_mode(1);
            mdss_dsi_panel_cmd_read(ctrl_pdata, pwr_cmd[0], pwr_cmd[1], NULL, rx_buf, 1);
            pr_err("[Display] %s: read power register 0x0A = 0x%02x\n", __func__, rx_buf[0]);
            i++;
        }
    } else
        set_alpm_mode(0);
...
static bool mdss_dsi_cmp_panel_reg_v2(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	for (i = 0; i < len; i++) {
		pr_err("[Display][%i] return:0x%x, status:0x%x\n",
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->hbm_off_cmds,
		"qcom,mdss-dsi-hbm-off-command", "qcom,mdss-dsi-lp-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->alpm_5nits_cmds,
		"qcom,mdss-dsi-alpm-5nits-command", "qcom,mdss-dsi-lp-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->alpm_40nits_cmds,
		"qcom,mdss-dsi-alpm-40nits-command", "qcom,mdss-dsi-lp-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->alpm_off_cmds,
		"qcom,mdss-dsi-alpm-off-command", "qcom,mdss-dsi-lp-command-state");
	...
	int ndx)
{
    ...
    proc_create(LCD_UNIQUE_ID, 0444, NULL, &lcd_uniqueID_ops);
