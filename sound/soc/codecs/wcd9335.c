...
{
	char messages[256];
	struct tasha_priv *tasha = snd_soc_codec_get_drvdata(registered_codec);
	...
	if (g_DebugMode)
		...
		switch (tasha->mbhc.current_plug) {
...
static void regmap_proc_free_dump_cache(struct regmap *map)
{
	...
	while (!list_empty(&registered_codec->component.regmap->proc_off_cache)) {
		c = list_first_entry(&registered_codec->component.regmap->proc_off_cache,
						  ...
						  loff_t *pos)
{
	 ...
	 */
	mutex_lock(&registered_codec->component.regmap->proc_cache_lock);
	i = base;
	if (list_empty(&registered_codec->component.regmap->proc_off_cache)) {
		printk("austin list_empty(&map->debugfs_off_cache austin regmap_debugfs_get_dump_start\n");
		for (; i <= registered_codec->component.regmap->max_register; i += registered_codec->component.regmap->reg_stride) {
			/* Skip unprinted registers, closing off cache entry */
			if (!regmap_readable(registered_codec->component.regmap, i) ||
			    regmap_precious(registered_codec->component.regmap, i)) {
				if (c) {
					c->max = p - 1;
					c->max_reg = i - registered_codec->component.regmap->reg_stride;
					list_add_tail(&c->list,
						      &registered_codec->component.regmap->proc_off_cache);
					c = NULL;
				}
			...
			if (!c) {
				...
				if (!c) {
					regmap_proc_free_dump_cache(registered_codec->component.regmap);
					mutex_unlock(&registered_codec->component.regmap->proc_cache_lock);
					printk("austin return base (%d)\n", base);
					return base;
				}
				c->min = p;
				c->base_reg = i;
	 ...
	 * no registers at all.
	 */
	WARN_ON(list_empty(&registered_codec->component.regmap->proc_off_cache));
	ret = base;

	/* Find the relevant block:offset */
	list_for_each_entry(c, &registered_codec->component.regmap->proc_off_cache, list) {
		if (from >= c->min && from <= c->max) {
			fpos_offset = from - c->min;
			reg_offset = fpos_offset / 9;
			*pos = c->min + (reg_offset * 9);
			mutex_unlock(&registered_codec->component.regmap->proc_cache_lock);
			printk("return %d austin \n", c->base_reg + (reg_offset * registered_codec->component.regmap->reg_stride));
			return c->base_reg + (reg_offset * registered_codec->component.regmap->reg_stride);
		}
...
static ssize_t audio_register_dump_proc_read(struct file *filp, char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	start_reg = regmap_proc_get_dump_start(registered_codec->component.regmap, from, *ppos, &p);

	printk("austin start_reg(%d) from(%d), to(%d)\n", start_reg, from, registered_codec->component.regmap->max_register);
	for (i = start_reg; i <= registered_codec->component.regmap->max_register; i += registered_codec->component.regmap->reg_stride) {
		if (!regmap_readable(registered_codec->component.regmap, i))
			continue;

		if (regmap_precious(registered_codec->component.regmap, i))
			continue;
		...
		if (p >= *ppos) {
			...
			ret = regmap_read(registered_codec->component.regmap, i, &val);
			if (ret == 0)
				...
				memset(buf + buf_pos, 'X',
				       2);
			buf_pos += 2 * registered_codec->component.regmap->format.val_bytes;
...
static ssize_t audio_codec_status_proc_read(struct file *filp, char __user *buff, size_t len, loff_t *off)
{
	...
	ret = regmap_read(registered_codec->component.regmap, WCD9335_CHIP_TIER_CTRL_CHIP_ID_BYTE0, &val);

	if (ret == 0 && val == 0x01 && codec_ver == WCD9335)
		sprintf(messages, "1\n");
	else
		sprintf(messages, "0\n");
...
static int tasha_codec_probe(struct snd_soc_codec *codec)
{
	...
	/* ASUS_BSP Paul --- */
	INIT_LIST_HEAD(&registered_codec->component.regmap->proc_off_cache);
	mutex_init(&registered_codec->component.regmap->proc_cache_lock);
