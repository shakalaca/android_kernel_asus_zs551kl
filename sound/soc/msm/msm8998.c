...
struct msm_asoc_mach_data {
	...
	struct device_node *prim_mi2s_gpio_p;
...
static const struct snd_soc_dapm_widget msm_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MIC("Handset Mic second", NULL),
	SND_SOC_DAPM_MIC("Handset Mic third", NULL),
				  ...
				  struct snd_pcm_hw_params *params)
{
	...
	pr_debug("%s: format = %d, rate = %d, dai_link->be_id = %d\n",
		  __func__, params_format(params), params_rate(params), dai_link->be_id);
...
static int msm_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	snd_soc_dapm_ignore_suspend(dapm, "Handset Mic second");
	snd_soc_dapm_ignore_suspend(dapm, "Handset Mic third");
...
static int msm_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	...
	if (++mi2s_intf_conf[index].ref_cnt == 1) {
		...
		if (index == PRIM_MI2S)
		{
			msm_cdc_pinctrl_select_active_state(pdata->prim_mi2s_gpio_p);
			printk("msm_cdc_pinctrl_select_active_state(pdata->prim_mi2s_gpio_p \n");
		}
	}
clk_off:
	if (IS_ERR_VALUE(ret))
...
static void msm_mi2s_snd_shutdown(struct snd_pcm_substream *substream)
{
	...
	if (--mi2s_intf_conf[index].ref_cnt == 0) {
		if (index == PRIM_MI2S)
		{
			msm_cdc_pinctrl_select_sleep_state(pdata->prim_mi2s_gpio_p);
			printk("msm_cdc_pinctrl_select_sleep_state(pdata->prim_mi2s_gpio_p \n");
		}
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		...
		pdata->prim_mi2s_gpio_p = of_parse_phandle(pdev->dev.of_node,
					"qcom,prim-mi2s-gpios", 0);
